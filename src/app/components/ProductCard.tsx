...
import React from 'react';
import { ShoppingBag, Star } from 'lucide-react';
import { IProduct, getThumbnail, getBasePrice } from '../../interfaces/product.interface';
...
export function ProductCard({ product, rating = 4.8, reviews = 124 }: ProductCardProps) {
  // ── Guard — bail out before ANY property access if product is missing ──
  if (!product || !product._id || !product.title || !product.sizes?.length) {
    return null;
  }

  const thumbnail = getThumbnail(product.image);
  const startingPrice = getBasePrice(product.sizes);
  ...
  return (
    ...
    >
      <Link
        ...
        className="block relative overflow-hidden bg-neutral-100 aspect-[4/5] mb-4 rounded-2xl"
      >
        {/* Badge */}
        <div className="absolute top-3 left-3 z-10 flex flex-col gap-2">
          {startingPrice > 500 && (
      ...
      <div className="flex flex-col flex-1 px-1">
        ...
        <Link to={`/product/${product._id}`}>
          <h3 className="text-lg font-medium text-neutral-900 leading-tight mb-1 hover:text-neutral-600 transition-colors line-clamp-1">
            {product.title}
          </h3>
        </Link>
        ...
        <div className="mt-auto flex items-center justify-between">
          <div>
            ...
            {product.sizes.length > 1 && (
              <p className="text-xs text-neutral-400">onwards</p>
            )}
          </div>
        </div>
      </div>
